...
export class NextAuthPlugin {
    ...
    async install(context) {
        ...
        try {
            ...
            return this.createErrorResult('Failed to install NextAuth.js', startTime, [], error);
        }
    }
    async uninstall(context) {
        ...
        try {
            ...
            return this.createErrorResult('Failed to uninstall NextAuth.js', startTime, [], error);
        }
    }
    async update(context) {
        ...
        try {
            ...
            return this.createErrorResult('Failed to update NextAuth.js', startTime, [], error);
        }
    }
    ...
    async createAuthConfiguration(context) {
        ...
        await fsExtra.writeFile(path.join(authLibDir, 'config.ts'), configContent);
        // Generate auth types
        const typesContent = this.generateAuthTypes();
        await fsExtra.writeFile(path.join(authLibDir, 'types.ts'), typesContent);
    }
    async createAPIRoutes(context) {
        ...
        await fsExtra.writeFile(path.join(apiRouteDir, 'route.ts'), routeContent);
    }
    async createAuthComponents(context) {
        const { projectPath } = context;
        ...
        await fsExtra.writeFile(path.join(authComponentsDir, 'auth-provider.tsx'), providerContent);
    }
    async generateUnifiedInterfaceFiles(context) {
        ...
        await fsExtra.writeFile(path.join(authLibDir, 'index.ts'), unifiedContent);
    }
...
//# sourceMappingURL=nextauth-plugin.js.map