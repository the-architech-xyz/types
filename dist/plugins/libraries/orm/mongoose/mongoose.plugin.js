...
import { MongooseConfigSchema, MongooseDefaultConfig } from './MongooseSchema.js';
...
export class MongoosePlugin {
        ...
        try {
            ...
            return {
                ...
                configs: [
                    {
                        file: '.env',
                        content: MongooseGenerator.generateEnvConfig(pluginConfig),
            ...
            return this.createErrorResult('Failed to install Mongoose ODM', startTime, [], error);
        }
    }
    async uninstall(context) {
        ...
        try {
            ...
            return this.createErrorResult('Failed to uninstall Mongoose ODM', startTime, [], error);
        }
    }
    async update(context) {
        ...
        try {
            ...
            return this.createErrorResult('Failed to update Mongoose ODM', startTime, [], error);
        }
    }
    ...
    async initializeMongooseConfig(context) {
        ...
        const connectionContent = MongooseGenerator.generateMongooseConnection(pluginConfig);
        await fsExtra.writeFile(path.join(dbLibDir, 'connection.ts'), connectionContent);
        // Generate User model with latest patterns
        const userModelContent = MongooseGenerator.generateUserModel(pluginConfig);
        await fsExtra.writeFile(path.join(modelsDir, 'user.model.ts'), userModelContent);
    }
    ...
    async createPluginsAndMiddleware(context) {
        ...
        await fsExtra.writeFile(path.join(pluginsDir, 'index.ts'), pluginsIndexContent);
        // Generate timestamp plugin
        const timestampPluginContent = MongooseGenerator.generateTimestampPlugin();
        await fsExtra.writeFile(path.join(pluginsDir, 'timestamp.plugin.ts'), timestampPluginContent);
        // Generate soft delete plugin
        const softDeletePluginContent = MongooseGenerator.generateSoftDeletePlugin();
        await fsExtra.writeFile(path.join(pluginsDir, 'soft-delete.plugin.ts'), softDeletePluginContent);
    }
