...
export class StructureService {
     ...
     */
    createStructureInfo(userPreference, template = 'nextjs-14') {
     ...
     */
    validateStructureInfo(info) {
        return ((info.type === 'monorepo' && info.userPreference === 'scalable-monorepo') ||
     ...
     */
    getPaths(projectPath, structure) {
        const paths = {
            root: projectPath,
            src: '',
            app: '',
     ...
     */
    getModulePath(projectPath, structure, moduleName) {
     ...
     */
    getAppPath(projectPath, structure, appName = 'web') {
        const paths = this.getPaths(projectPath, structure);
     ...
     */
    getUnifiedInterfacePath(projectPath, structure, moduleName) {
        const paths = this.getPaths(projectPath, structure);
        if (structure.isMonorepo) {
     ...
     */
    async transformToMonorepo(projectPath) {
     ...
     */
    async transformToSingleApp(projectPath) {
     ...
     */
    async pathExists(projectPath, structure, relativePath) {
     ...
     */
    async ensureModuleDirectory(projectPath, structure, moduleName) {
