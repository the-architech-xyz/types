...
export class KeywordAnalyzer {
    ...
    static analyzeInput(userInput) {
        ...
        for (const pattern of KEYWORD_PATTERNS) {
            const matchedWords = pattern.keywords.filter(keyword => words.some(word => word.includes(keyword) || keyword.includes(word)));
     ...
     */
    static calculateConfidence(score, totalWords) {
     ...
     */
    static generateReason(keywords, template) {
     ...
     */
    static getSuggestionsByComplexity(complexity) {
     ...
     */
    static getSuggestionsByAudience(audience) {
     ...
     */
    static extractThemes(userInput) {
        if (!userInput || userInput.trim().length === 0) {
            return [];
        }
        const input = userInput.toLowerCase();
     ...
     */
    static suggestsCustomSetup(userInput) {
        if (!userInput || userInput.trim().length === 0) {
            return false;
        }
        const input = userInput.toLowerCase();
...
//# sourceMappingURL=keyword-analyzer.js.map