...
export class TemplateService {
     ...
     */
    async render(templatePath, data, options = {}) {
     ...
     */
    async renderTemplateNew(category, templatePath, data, options = {}) {
     ...
     */
    async renderFrameworkTemplate(framework, templatePath, data, options = {}) {
        ...
        try {
            ...
            const errorMessage = error instanceof Error ? error.message : 'Unknown error';
            this.logger?.error(`Failed to render framework template ${templatePath}: ${errorMessage}`, error);
     ...
     */
    async renderAndWriteNew(category, templatePath, outputPath, data, options = {}) {
        try {
            this.logger?.info(`Rendering and writing: ${templatePath} -> ${outputPath}`);
     ...
     */
    async renderFrameworkAndWrite(framework, templatePath, outputPath, data, options = {}) {
        try {
            ...
            const errorMessage = error instanceof Error ? error.message : 'Unknown error';
            this.logger?.error(`Failed to render and write framework template ${templatePath}: ${errorMessage}`, error);
            throw error;
        }
    }
    /**
     ...
     */
    async templateExistsNew(category, templatePath) {
     ...
     */
    async frameworkTemplateExists(framework, templatePath) {
     ...
     */
    async listTemplatesNew(category) {
     ...
     */
    async getTemplateContentNew(category, templatePath) {
        const fullPath = path.join(this.templatesRoot, category, templatePath);
        if (!await fs.pathExists(fullPath)) {
            throw new Error(`Template not found: ${fullPath}`);
        }
