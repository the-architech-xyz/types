...
export class OrchestratorAgent {
    ...
    parseUserRequirements(userInput, projectName) {
        ...
        if (input.includes('vue'))
            requirements.ui.framework = 'vue';
        if (input.includes('angular'))
            requirements.ui.framework = 'angular';
        if (input.includes('react') && !input.includes('next'))
            requirements.ui.framework = 'react';
        // Database detection
        if (input.includes('mysql'))
            requirements.database.type = 'mysql';
        if (input.includes('sqlite'))
            requirements.database.type = 'sqlite';
        if (input.includes('mongodb'))
            requirements.database.type = 'mongodb';
        if (input.includes('prisma'))
            requirements.database.orm = 'prisma';
        // Authentication detection
        if (input.includes('github'))
            requirements.authentication.providers.push('github');
        if (input.includes('google'))
            requirements.authentication.providers.push('google');
        if (input.includes('oauth'))
            requirements.authentication.providers.push('oauth');
        // Deployment detection
        if (input.includes('netlify'))
            requirements.deployment.platform = 'netlify';
        if (input.includes('railway'))
            requirements.deployment.platform = 'railway';
        if (input.includes('aws'))
            requirements.deployment.platform = 'aws';
        // Testing detection
        if (input.includes('vitest'))
            requirements.testing.framework = 'vitest';
        if (input.includes('playwright'))
            requirements.testing.framework = 'playwright';
        return requirements;
    }
    async enhanceRequirements(requirements, context) {
        ...
        for (const pluginId of phase.plugins) {
            ...
            if (plugin) {
                ...
                if (result.success) {
                    ...
                    warnings.push(`Plugin ${pluginId} failed: ${result.errors.map((e) => e.message).join(', ')}`);
                }
            }
        }
