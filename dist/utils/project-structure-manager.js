...
export class ProjectStructureManager {
     ...
     */
    async createStructure(projectPath, config, templateData) {
        ...
        try {
            ...
            this.logger?.error(`Failed to create project structure: ${errorMessage}`, error);
            throw error;
        }
    }
    /**
     ...
     */
    async createDirectories(projectPath, structure) {
     ...
     */
    async createFiles(projectPath, structure, config, templateData) {
        // Create root files
     ...
     */
    async createRootFiles(projectPath, files, config, templateData) {
        ...
        for (const file of files) {
            ...
            try {
                ...
                this.logger?.error(`Failed to create root file ${file}: ${error instanceof Error ? error.message : 'Unknown error'}`, error);
            }
        }
    }
    /**
     ...
     */
    async createAppFiles(projectPath, apps, config, templateData) {
     ...
     * Create framework-specific app files
     */
    async createFrameworkAppFiles(appPath, framework, templateData) {
     ...
     */
    async createSingleAppFrameworkFiles(projectPath, framework, templateData) {
        if (framework === 'nextjs' || framework === 'nextjs-14') {
     ...
     */
    async createPackageFiles(projectPath, packages, config, templateData) {
     ...
     * Create shared files
     */
    async createSharedFiles(projectPath, files, config, templateData) {
     ...
     */
    async validateStructure(projectPath, config) {
        try {
            const directories = this.getDirectoryStructure(config);
            const files = this.getFileStructure(config);
...
//# sourceMappingURL=project-structure-manager.js.map