 ...
 *
 * Provides CLI commands for managing plugins:
...
import { PluginAdapter } from '../utils/plugin-adapter.js';
    ...
    command
        ...
        try {
            ...
            if (options.category) {
                ...
                plugins = pluginSystem.getRegistry().getPluginsByTag(options.tag);
            }
            else {
...
async function interactiveConfig(plugin) {
    const configSchema = plugin.getConfigSchema();
    ...
    for (const [key, prop] of Object.entries(configSchema.properties)) {
        ...
        if (typedProp.type === 'array' && typedProp.items?.enum) {
            ...
            question.choices = typedProp.items.enum.map((option) => ({
                name: option,
                value: option,
