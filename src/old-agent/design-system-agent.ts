...
export class DesignSystemAgent extends AbstractAgent {
  ...
  protected async executeInternal(context: AgentContext): Promise<AgentResult> {
    ...
    try {
      ...
      context.logger.error(`Failed to configure design system: ${errorMessage}`, error as Error);
      
      return this.createErrorResult(
        'DESIGN_SYSTEM_SETUP_FAILED',
        `Failed to configure design system: ${errorMessage}`,
        [],
        0,
        error
      );
  ...
  private async createShadcnConfig(context: AgentContext): Promise<Artifact[]> {
  ...
  private async installCoreComponents(context: AgentContext): Promise<Artifact[]> {
  ...
  private async createUtilsLib(context: AgentContext): Promise<Artifact[]> {
...
`;
  ...
  private async createComponentStructure(context: AgentContext): Promise<Artifact[]> {
