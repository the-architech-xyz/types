...
import { AgentContext, ValidationResult, ValidationError, AgentResult, AgentMetadata, AgentCapability, AgentCategory, CapabilityCategory } from '../types/agent.js';
...
export class ValidationAgent extends AbstractAgent {
  ...
  private async runValidationChecks(projectPath: string, context: AgentContext): Promise<ValidationResults> {
  ...
  private async validateProjectStructure(projectPath: string): Promise<ValidationCategory> {
    const issues: string[] = [];
    const requiredDirs = [
      'apps/web',
      'packages/ui',
  ...
  private async validateDependencies(projectPath: string, context: AgentContext): Promise<ValidationCategory> {
    const issues: string[] = [];
    
    try {
      const rootPackageJson = await fsExtra.readJSON(path.join(projectPath, 'package.json')) as any;
      const webPackageJson = await fsExtra.readJSON(path.join(projectPath, 'apps', 'web', 'package.json')) as any;
  ...
  private async validateConfiguration(projectPath: string): Promise<ValidationCategory> {
    const issues: string[] = [];
    
    try {
      // Validate TypeScript config
      const tsConfig = await fsExtra.readJSON(path.join(projectPath, 'tsconfig.json')) as any;
      if (!tsConfig.compilerOptions?.paths) {
        issues.push('TypeScript paths not configured');
      }

      // Validate ESLint config
      const eslintConfig = await fsExtra.readJSON(path.join(projectPath, '.eslintrc.json')) as any;
      ...
      const prettierConfig = await fsExtra.readJSON(path.join(projectPath, '.prettierrc.json')) as any;
      ...
      const componentsConfig = await fsExtra.readJSON(path.join(projectPath, 'components.json')) as any;
  ...
  private async validateImportPaths(projectPath: string, context: AgentContext): Promise<ValidationCategory> {
    const issues: string[] = [];
  ...
  private async validateTypeScript(projectPath: string): Promise<ValidationCategory> {
    const issues: string[] = [];
    ...
    try {
      ...
      try {
        ...
        if (error instanceof Error && 'exitCode' in error && (error as any).exitCode !== 0) {
