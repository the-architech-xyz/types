...
export class DBAgent extends AbstractAgent {
  ...
  protected async executeInternal(context: AgentContext): Promise<AgentResult> {
    ...
    try {
      ...
      context.logger.error(`Failed to setup database package: ${errorMessage}`, error as Error);
  ...
  private async executeDrizzlePlugin(context: AgentContext, dbConfig: DatabaseConfig) {
  ...
  private async writePluginArtifacts(artifacts: any[], context: AgentContext): Promise<void> {
  ...
  private async enhanceDatabaseSetup(
  ...
  private async generateCustomSchema(
    dbPackagePath: string, 
    dbConfig: DatabaseConfig, 
    context: AgentContext
  ) {
...
export type { DatabaseSchema } from './custom';`;
  ...
  private async generateSchemaContent(schema: string[], context: AgentContext): Promise<string> {
...
`;
  ...
  private generateTableDefinition(tableName: string): string {
...
${baseFields},
  ...
  private async createEnhancedUtils(dbPackagePath: string, context: AgentContext): Promise<void> {
...
export class DatabaseUtils {
  ...
  private async createHealthChecks(dbPackagePath: string, context: AgentContext): Promise<void> {
    const utilsDir = path.join(dbPackagePath, 'utils');
    await fsExtra.ensureDir(utilsDir);
...
export class DatabaseHealth {
  ...
  private async createSeedingUtils(dbPackagePath: string, context: AgentContext): Promise<void> {
    const utilsDir = path.join(dbPackagePath, 'utils');
    await fsExtra.ensureDir(utilsDir);
...
export class DatabaseSeeder {
  ...
  async rollback(context: AgentContext): Promise<void> {
    ...
    try {
      ...
      const errorMessage = error instanceof Error ? error.message : 'Unknown error';
      context.logger.error(`Failed to remove database package: ${errorMessage}`, error as Error);
    }
  }
} 