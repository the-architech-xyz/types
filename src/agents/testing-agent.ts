...
export class TestingAgent extends AbstractAgent {
  ...
  private async executeTestingPluginUnified(
  ...
  private async validateTestingSetupUnified(
    context: AgentContext,
    pluginName: string
  ): Promise<void> {
    try {
  ...
  private async selectTestingPlugin(context: AgentContext, testingConfig: TestingConfig): Promise<string> {
  ...
  private getPluginConfig(testingConfig: TestingConfig, pluginName: string): Record<string, any> {
  ...
  async rollback(context: AgentContext): Promise<void> {
    ...
    try {
      ...
      context.logger.error('Testing rollback failed', error as Error);
    }
  }
} 