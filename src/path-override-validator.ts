...
import type { MarketplacePathKeys, MarketplacePathKeyDefinition } from './path-keys.js';
 ...
 */
export async function validatePathOverrides(
  overrides: Record<string, string>,
  marketplacePathKeys: MarketplacePathKeys,
  projectStructure?: 'monorepo' | 'single-app'
): Promise<PathOverrideValidationResult> {
  const errors: Array<{ key: string; message: string }> = [];
  const warnings: Array<{ key: string; message: string }> = [];
  
  // Create a set of valid path keys for quick lookup
  const validKeys = new Set<string>();
  const keyDefinitions = new Map<string, MarketplacePathKeyDefinition>();
