...
export interface ModuleMetadata {
  ...
  source: { root: string; marketplace: string };
  manifest: { file: string };
  blueprint: { file: string; runtime: 'source' | 'compiled' };
  templates: string[];
...
export interface ResolvedModule extends Module {
  // Same structure as Module, but guaranteed complete after transformation
}
...
export interface ResolvedGenome extends Genome {
  modules: ResolvedModule[];  // Required (never empty after transformation)
  metadata: ResolvedGenomeMetadata;  // Required (always present after transformation)
}
