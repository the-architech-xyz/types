...
async function executeAgents(config, runner) {
  const projectPath = path.resolve(config.projectName);
  ...
  const baseAgent = new BaseProjectAgent(runner);
  ...
  if (config.modules?.includes('best-practices')) {
    ...
    const practicesAgent = new BestPracticesAgent(runner);
    await practicesAgent.execute({ projectPath });
  ...
  if (config.modules?.includes('design-system')) {
    ...
    const designAgent = new DesignSystemAgent(runner);
    await designAgent.execute({ projectPath });
  ...
  if (config.modules?.includes('deployment')) {
    ...
    const deploymentAgent = new DeploymentAgent(runner);
    await deploymentAgent.execute({ projectPath });
