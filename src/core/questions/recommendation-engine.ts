...
export class RecommendationEngine {
  ...
  getRecommendations(context: ProjectContext): RecommendationSet {
    const recommendations: Partial<RecommendationSet> = {};
  ...
  getAlternatives(category: string): Recommendation[] {
    const alternatives: Record<string, Recommendation[]> = {
      database: [
        {
          category: 'database',
          plugin: 'drizzle',
  ...
  private getDatabaseRecommendation(context: ProjectContext): Recommendation {
  ...
  private getAuthRecommendation(context: ProjectContext): Recommendation {
    const { type, requirements } = context;
  ...
  private getUIRecommendation(context: ProjectContext): Recommendation {
    const { type, requirements } = context;
  ...
  private getPaymentRecommendation(context: ProjectContext): Recommendation {
    return {
      category: 'payment',
      plugin: 'stripe',
  ...
  private getEmailRecommendation(context: ProjectContext): Recommendation {
    return {
      category: 'email',
      plugin: 'resend',
      reason: 'Developer-friendly email API with great deliverability',
  ...
  private getTestingRecommendation(context: ProjectContext): Recommendation {
    return {
      category: 'testing',
      plugin: 'vitest',
      reason: 'Fast unit testing framework with excellent Vite integration',
  ...
  private getDeploymentRecommendation(context: ProjectContext): Recommendation {
    return {
      category: 'deployment',
      plugin: 'vercel',
