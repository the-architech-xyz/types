...
export class DynamicQuestionGenerator {
  ...
  generateQuestions(plugin: IEnhancedPlugin, context: PluginContext): PluginQuestion[] {
    ...
    const questions: PluginQuestion[] = [];
  ...
  private createQuestion(
    ...
    const baseProps = {
      ...
      order: param.order || 0,
      when: (answers: Record<string, any>) => this.evaluateConditions(param.conditions || [], answers)
    };

    // Add optional properties
    if (param.group) {
      (baseProps as any).group = param.group;
    }
    ...
    switch (param.type) {
      ...
      case 'number':
        return {
          ...baseProps,
          type: 'input',
          validate: (input: any) => this.validateParameter(param, input, {})
        };
      ...
      case 'object':
        ...
        return {
          ...
          validate: (input: any) => this.validateJSON(input)
        };
   ...
   */
  private createChoices(options: any[]): QuestionChoice[] {
   ...
   * Check if parameter should be shown based on conditions
   */
  private shouldShowParameter(param: ParameterDefinition, context: PluginContext): boolean {
   ...
   * Validate JSON input
   */
  private validateJSON(input: string): boolean | string {
    try {
      JSON.parse(input);
      return true;
    } catch {
   ...
   * Sort parameters by group and order
   */
  private sortParametersByGroup(schema: ParameterSchema): ParameterDefinition[] {
    const sorted: ParameterDefinition[] = [];
  ...
  private getGenericDatabaseQuestions(): PluginQuestion[] {
    return [
      {
        id: 'database_enabled',
        type: 'confirm',
  ...
  private getGenericAuthQuestions(): PluginQuestion[] {
    return [
      {
        id: 'auth_enabled',
        type: 'confirm',
  ...
  private getGenericUIQuestions(): PluginQuestion[] {
    return [
      {
        id: 'ui_enabled',
        type: 'confirm',
  ...
  private getGenericDeploymentQuestions(): PluginQuestion[] {
    return [
      {
  ...
  private getGenericTestingQuestions(): PluginQuestion[] {
    return [
      {
        id: 'testing_enabled',
        type: 'confirm',
  ...
  private getGenericEmailQuestions(): PluginQuestion[] {
    return [
      {
        id: 'email_enabled',
        type: 'confirm',
  ...
  private getGenericMonitoringQuestions(): PluginQuestion[] {
    return [
      {
  ...
  private getGenericPaymentQuestions(): PluginQuestion[] {
    return [
      {
        id: 'payment_enabled',
        type: 'confirm',
  ...
  private getGenericBlockchainQuestions(): PluginQuestion[] {
    return [
      {
