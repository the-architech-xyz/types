...
export class SendGridPlugin implements IPlugin {
  ...
  async uninstall(context: PluginContext): Promise<PluginResult> {
    ...
    try {
      ...
      // Remove dependencies
      await this.runner.install(['@sendgrid/mail', '@react-email/components', '@react-email/render'], false, projectPath);
  ...
  async validate(context: PluginContext): Promise<ValidationResult> {
    ...
    try {
      ...
      // Check if SendGrid configuration exists
      const sendgridConfigPath = path.join(projectPath, 'src', 'lib', 'email', 'client.ts');
      if (!await fsExtra.pathExists(sendgridConfigPath)) {
        errors.push({
          field: 'sendgrid-config',
          message: 'SendGrid client configuration not found',
          code: 'MISSING_CONFIG',
          severity: 'error'
        });
      }
      ...
      if (await fsExtra.pathExists(envPath)) {
        ...
        warnings.push('.env.local file not found - SendGrid environment variables need to be configured');
      }
      ...
      if (await fsExtra.pathExists(packageJsonPath)) {
        ...
        const requiredDeps = ['@sendgrid/mail'];
      ...
      if (majorVersion < 16) {
        errors.push({
          field: 'node-version',
          message: 'Node.js version 16 or higher is required for SendGrid',
    ...
    return [
      {
        ...
        description: 'A SendGrid account with API key',
        optional: false
      },
      {
        type: 'config',
        name: 'Environment Variables',
        description: 'SENDGRID_API_KEY and EMAIL_FROM must be configured',
        optional: false
      },
      {
        type: 'binary',
  ...
  private async createEmailConfiguration(context: PluginContext): Promise<void> {
    ...
    context.logger.info('Creating SendGrid email configuration...');
  ...
  private async createEmailTemplates(context: PluginContext): Promise<void> {
    ...
    context.logger.info('Creating SendGrid email templates...');
    ...
    await fsExtra.writeFile(
      path.join(emailsDir, 'verification.tsx'),
      verificationContent
    );
    ...
    await fsExtra.writeFile(
      path.join(emailsDir, 'reset-password.tsx'),
    ...
    await fsExtra.writeFile(
      path.join(projectPath, 'src', 'lib', 'email', 'templates.ts'),
      templatesContent
    );
  }
  ...
  private async createAPIRoutes(context: PluginContext): Promise<void> {
    ...
    context.logger.info('Creating SendGrid API routes...');
  ...
  private async createEmailUtilities(context: PluginContext): Promise<void> {
    ...
    context.logger.info('Creating SendGrid email utilities...');
  ...
  private generateEmailClient(): string {
...
`;
  ...
  private generateEmailConfig(config: Record<string, any>): string {
    return `/**
 * Email Configuration
 * Generated by The Architech SendGrid Plugin
 */
...
export const emailConfig: EmailConfig = {
  ...
  features: {
    ...
    marketing: ${config.features?.marketing || false}
  }
};
...
`;
  ...
  private generateEmailTypes(): string {
    return `import sgMail from '@sendgrid/mail';
...
`;
  ...
  private generateResetPasswordTemplate(): string {
    return `import {
  Body,
  Container,
  Head,
...
export async function POST(request: NextRequest) {
  try {
    ...
    switch (body.event) {
      case 'delivered':
        console.log('Email delivered:', body);
        break;
      case 'open':
        console.log('Email opened:', body);
        break;
      case 'click':
        console.log('Email clicked:', body);
        break;
      case 'bounce':
        console.log('Email bounced:', body);
        break;
      case 'unsubscribe':
        console.log('Email unsubscribed:', body);
        break;
      default:
...
`;
  ...
  private generateEmailService(): string {
...
export class EmailService {
  ...
  static async getEmailStats(): Promise<any> {
    // This would typically call SendGrid's API to get statistics
...
`;
  ...
  private generateUnifiedIndex(): string {
    return `/**
 * SendGrid Email Unified Interface
 * 
 * This file provides a unified interface for all SendGrid email functionality
 * including sending emails, templates, validation, and analytics.
 */

// Core exports
export { default as sgMail } from './client';
