...
import { ShadcnUIConfig, ShadcnUIConfigSchema, ShadcnUIDefaultConfig } from './ShadcnUISchema.js';
...
export class ShadcnUIPlugin implements IPlugin {
    ...
    try {
      ...
      return {
        ...
        configs: [
          {
            file: '.env',
            content: ShadcnUIGenerator.generateEnvConfig(pluginConfig as ShadcnUIConfig),
  ...
  private async createTailwindConfig(context: PluginContext): Promise<void> {
    ...
    const tailwindConfig = ShadcnUIGenerator.generateTailwindConfig(pluginConfig as ShadcnUIConfig);
    ...
    const cssVariables = ShadcnUIGenerator.generateCSSVariables(pluginConfig as ShadcnUIConfig);
    await fsExtra.writeFile(
