...
export class DrizzleGenerator {
  ...
  generateEnvVars(config: DatabasePluginConfig): Record<string, string> {
    ...
    if (config.provider === DATABASE_PROVIDERS.SUPABASE) {
      const supabaseConfig = config as any; // Assuming SupabaseConfig is not directly imported here
    ...
    if (config.provider === DATABASE_PROVIDERS.MONGODB) {
      const mongoConfig = config as any; // Assuming MongoDBConfig is not directly imported here
