...
export class NextJSPlugin implements IPlugin {
  ...
  private async createProjectStructure(context: PluginContext): Promise<void> {
  ...
  private generateNextConfig(context: PluginContext): string {
...
`;
  ...
  private generateTsConfig(context: PluginContext): string {
    return `{
  "compilerOptions": {
    "target": "es5",
  ...
  private generateEslintConfig(context: PluginContext): string {
    return `{
  ...
  private generateProjectStructure(context: PluginContext): Record<string, string> {
    const useAppRouter = context.pluginConfig.appRouter !== false;
