...
export class SentryPlugin implements IPlugin {
  ...
  async uninstall(context: PluginContext): Promise<PluginResult> {
    ...
    try {
      ...
      return this.createErrorResult(
        'Failed to uninstall Sentry monitoring',
        startTime,
        [],
        error
      );
  ...
  async update(context: PluginContext): Promise<PluginResult> {
    ...
    try {
      ...
      return this.createErrorResult(
        'Failed to update Sentry monitoring',
        startTime,
        [],
        error
      );
  ...
  private async initializeSentryConfig(context: PluginContext): Promise<void> {
  ...
  private async createMonitoringFiles(context: PluginContext): Promise<void> {
    const { projectPath } = context;
    ...
    await fsExtra.writeFile(
      path.join(monitoringDir, 'utilities.ts'),
      utilitiesContent
    );
  }
  ...
  private async generateUnifiedInterfaceFiles(context: PluginContext): Promise<void> {
    ...
    await fsExtra.writeFile(
      path.join(monitoringDir, 'index.ts'),
      unifiedContent
    );
  }

  private generateSentryClientConfig(config: Record<string, any>): string {
...
Sentry.init({
  ...
  private generateSentryServerConfig(config: Record<string, any>): string {
    const enablePerformance = config.enablePerformance !== false;
...
Sentry.init({
  ...
  private generateSentryMonitoring(config: Record<string, any>): string {
    return `import * as Sentry from '@sentry/nextjs';
...
`;
  ...
  private generateSentryProperties(config: Record<string, any>): string {
