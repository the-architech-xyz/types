...
import { ORM_LIBRARIES, DATABASE_PROVIDERS, DATABASE_ORM_COMPATIBILITY } from '../../../types/shared-config.js';
...
export class DrizzlePlugin implements IPlugin {
  ...
  async validate(context: PluginContext): Promise<ValidationResult> {
    try {
      ...
      if (config?.databaseProvider) {
        const compatibleORMs = DATABASE_ORM_COMPATIBILITY[config.databaseProvider as keyof typeof DATABASE_ORM_COMPATIBILITY];
      ...
      const depValidation = await PluginValidationUtils.validateDependencies(
        context.projectPath,
      ...
      return PluginValidationUtils.createErrorResult(
        `Drizzle plugin validation failed: ${error instanceof Error ? error.message : 'Unknown error'}`,
        'drizzle.validation'
      );
    }
  }
  ...
  private async createORMFiles(context: PluginContext): Promise<void> {
    const { projectPath } = context;
...
`;
  ...
  private generateORMConnection(): string {
