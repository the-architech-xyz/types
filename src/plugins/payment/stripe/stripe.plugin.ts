...
export class StripePlugin implements IPlugin {
  ...
  private async initializeStripeConfig(context: PluginContext): Promise<void> {
    ...
    await fsExtra.writeFile(
      path.join(paymentDir, 'stripe.ts'),
      stripeContent
    );
    ...
    await fsExtra.writeFile(
      path.join(paymentIntentDir, 'route.ts'),
  ...
  private async createPaymentFiles(context: PluginContext): Promise<void> {
    ...
    await fsExtra.writeFile(
      path.join(paymentDir, 'utilities.ts'),
      utilitiesContent
    );
  }
  ...
  private async generateUnifiedInterfaceFiles(context: PluginContext): Promise<void> {
    ...
    await fsExtra.writeFile(
      path.join(paymentDir, 'index.ts'),
      unifiedContent
    );
  }

  private generateStripeClient(config: Record<string, any>): string {
...
`;
  ...
  private generateWebhookRoute(config: Record<string, any>): string {
...
`;
  ...
  private generatePaymentIntentRoute(config: Record<string, any>): string {
    return `import { NextRequest, NextResponse } from 'next/server';
...
`;
  ...
  private generatePaymentUtilities(): string {
    return `/**
 * Payment Utilities
 * 
