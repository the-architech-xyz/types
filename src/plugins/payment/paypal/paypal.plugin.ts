...
export class PayPalPlugin implements IPlugin {
  ...
  async install(context: PluginContext): Promise<PluginResult> {
    ...
    try {
      ...
      this.logger.error('Failed to install PayPal plugin:', error as Error);
      return {
        success: false,
        artifacts: [],
        dependencies: [],
  ...
  private generatePayPalConfig(config: Record<string, any>): string {
...
`;
  ...
  private generatePayPalProvider(config: Record<string, any>): string {
    return `'use client';
...
`;
  ...
  private generatePayPalUtils(config: Record<string, any>): string {
    const enableSubscriptions = config.enableSubscriptions === true;
    const enableInvoices = config.enableInvoices === true;
...
`;
  ...
  private generateCreateOrderRoute(config: Record<string, any>): string {
...
`;
  ...
  private generateCaptureOrderRoute(config: Record<string, any>): string {
    return `import { NextRequest, NextResponse } from 'next/server';
import paypal from '@paypal/checkout-server-sdk';
...
`;
  ...
  private generateSubscriptionsRoute(config: Record<string, any>): string {
    return `import { NextRequest, NextResponse } from 'next/server';
import paypal from '@paypal/checkout-server-sdk';
