...
export class ShadcnUIPlugin implements IPlugin {
  ...
  private getRadixDependencies(components: string[]): DependencyInfo[] {
  ...
  private generateTailwindConfig(context: PluginContext): string {
...
module.exports = {
  ...
  private generateGlobalCSS(context: PluginContext): string {
...
@layer base {
  * {
  ...
  private generateComponentsConfig(context: PluginContext): string {
    return `{
  ...
  private generateUtils(context: PluginContext): string {
...
export function cn(...inputs: ClassValue[]) {
  ...
  private generateComponentStructure(context: PluginContext, components: string[]): Record<string, string> {
...
export { ${componentName.charAt(0).toUpperCase() + componentName.slice(1)} }`;
  ...
  private generatePackageJsonUpdates(context: PluginContext): string {
