...
export class DockerPlugin implements IPlugin {
  ...
  private async createDeploymentScripts(context: PluginContext): Promise<void> {
    ...
    await fsExtra.writeFile(
      path.join(scriptsDir, 'docker-build.sh'),
      buildScriptContent
    );
    ...
    await fsExtra.writeFile(
      path.join(scriptsDir, 'docker-deploy.sh'),
      deployScriptContent
    );
  ...
  private async generateUnifiedInterfaceFiles(context: PluginContext): Promise<void> {
    ...
    await fsExtra.writeFile(
      path.join(deployDir, 'index.ts'),
      unifiedContent
    );
  }

  private generateDockerfile(config: Record<string, any>): string {
...
temp`;
  ...
  private generateDockerCompose(config: Record<string, any>): string {
...
volumes:
  ...
  private generateBuildScript(config: Record<string, any>): string {
    const imageName = config.imageName || 'myapp';
    const imageTag = config.imageTag || 'latest';
...
echo "🎉 Build completed!"`;
  ...
  private generateDeployScript(config: Record<string, any>): string {
    const imageName = config.imageName || 'myapp';
    const imageTag = config.imageTag || 'latest';
...
echo "🎉 Deployment completed!"`;
  ...
  private generateK8sDeployment(config: Record<string, any>): string {
    const imageName = config.imageName || 'myapp';
    const imageTag = config.imageTag || 'latest';
    const port = config.port || 3000;
...
spec:
  ...
  private generateK8sService(config: Record<string, any>): string {
    const imageName = config.imageName || 'myapp';
    const port = config.port || 3000;
...
spec:
  ...
  private generateK8sIngress(config: Record<string, any>): string {
    const imageName = config.imageName || 'myapp';
