...
export class SupabasePlugin implements IPlugin {
  ...
  private async initializeSupabaseConfig(context: PluginContext): Promise<void> {
    ...
    await fsExtra.writeFile(
      path.join(supabaseDir, 'migrations', '00000000000000_initial_schema.sql'),
      migrationContent
    );
  }
  ...
  private async createDatabaseFiles(context: PluginContext): Promise<void> {
    ...
    await fsExtra.writeFile(
      path.join(supabaseDir, 'database.ts'),
      databaseContent
    );
    ...
    await fsExtra.writeFile(
      path.join(supabaseDir, 'schema.ts'),
      schemaContent
    );
  }
  ...
  private async createSupabaseClient(context: PluginContext): Promise<void> {
    const { projectPath } = context;
    ...
    await fsExtra.writeFile(
      path.join(supabaseDir, 'client.ts'),
      clientContent
    );
    ...
    await fsExtra.writeFile(
      path.join(supabaseDir, 'types.ts'),
      typesContent
    );
  }
  ...
  private generateSupabaseConfig(config: Record<string, any>): string {
...
`;
  ...
  private generateInitialMigration(config: Record<string, any>): string {
...
`;
  ...
  private generateAuthUtilities(config: Record<string, any>): string {
    return `import { supabase } from './client';
...
`;
  ...
  private generateAuthCallbackRoute(): string {
...
`;
  ...
  private generateSchemaTypes(config: Record<string, any>): string {
