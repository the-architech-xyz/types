...
import { DATABASE_PROVIDERS, ORM_LIBRARIES } from '../../../types/shared-config.js';
...
export class NeonPlugin implements IPlugin {
  ...
  async uninstall(context: PluginContext): Promise<PluginResult> {
    ...
    try {
      ...
      return this.createErrorResult(
        `Failed to uninstall Neon database: ${errorMessage}`,
        startTime,
        [],
        error
      );
  ...
  private generateNeonConfig(config: Record<string, any>): string {
...
`;
  ...
  private generateNeonConnection(): string {
